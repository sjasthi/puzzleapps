<?php
   ...
   $query = "SELECT *
      FROM puzzlecreator";
   $GLOBALS['puzzleCreatorResults'] = mysqli_query($db, $query);
      ...
      <h1>Puzzle Game Table</h1>
        ...
        <div id="puzzleFields">
               <form class="form" action="createPuzzleCreator.php" method="POST">
                  ...
                  <button type="submit" name="submit" class="btn btn-success btn-sm">Add Puzzle Creator</button>
                  <br>
                  <br>
               </form>
            </div>
            ...
            <table id="info" cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered" width="100%">
                  <thead>
                     <tr>
                        ...
                        <th>Puzzle Creator Name</th>
                        <th>Puzzle Creator Description</th>
                        <th>Puzzle Creator Notes</th>
                        <th>Puzzle Creator Developer</th>
                        <th>Puzzle Creator Actions</th>
                        
                     </tr>
                  </thead>
                  <tbody>
                      <?php
                        if ($puzzleCreatorResults->num_rows > 0) {
                            // output data of each row
                            while($row = $puzzleCreatorResults->fetch_assoc()) {
                                echo '<tr>
                                          <td> 
                                          ...
                                          <td> '.$row["puzzleCreatorDescription"].'</td>
                                          <td> '.$row["notes"].'</td>
                                          <td> '.$row["developer"].'</td>
